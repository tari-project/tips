...
pub struct SpendScript {
    ...
    /// parameter**, matching the engine's `Vec<Bytes>` call ABI. Baked into the output at
...
(e.g. a single `timelock` function bound with different unlock epochs per output). An output may
also commit to a *set* of alternative scripts under a single root rather than one inline script,
revealing only the path taken at spend time — see §9 (script trees).
...
> `script_root`, and a `SpendScript` is carried as a script-tree leaf (`id = 0`). §1–§8 describe the
> per-script mechanism, which is unchanged; it now applies to the leaf reached via the script-path
> witness. The `SpendScript` shape above is retained verbatim as that leaf payload.
...
mod vault_covenant {
    pub fn enforce(unlock_epoch: u64, ctx: SpendContext) {
        ...
        assert!(
            ...
            "outputs must preserve the spend condition",
        );
    }
}
```
...
| Bound args | the **leading** arguments (all but the last) are the bound parameters. `SpendScript.args` is **positional** — one CBOR-encoded value per leading parameter — so `args.len()` must equal `func.arguments.len() - 1`, and element `i` must decode to leading parameter `i` |
...
fn validate_spend_script_signature(func: &FunctionDef) -> Result<(), RuntimeError> {
    ...
    // the leading args (all but the last) are the bound-arg types `SpendScript.args` decodes to
    Ok(())
}
```
...
a `script_root` output (§10) the leaf to evaluate is the one named by the input's script-path
...
2. For `Script`, the engine resolves the `LoadedTemplate`, re-runs
...
A `SpendScript` — `template`, `function`, and especially `args` — is committed into the output
created by a `StealthTransfer` instruction, so its serialized size **must contribute to the
...
extends it so each output additionally contributes the byte length of its `SpendCondition`
(notably the `Script` `args`). Without this, a large script/args could be broadcast for free and
...
#### 9. Script trees (MAST): committing to a set of alternative scripts

The §1 `Script` variant commits to exactly one predicate, in the clear. Many real conditions are
...
A **Merklized script tree (MAST)** lets an output commit to a *set* of alternative spend scripts
under a single 32-byte root. The output stores only the root; at spend time the spender reveals the
one leaf they are exercising plus a Merkle inclusion proof, and the engine:
...
**Leaf format.** A leaf is `(id, payload)`: `id` is a type/version discriminant and `payload` its
body. `id = 0` is a CBOR-encoded `SpendScript` (§1); reserving `id` keeps the tree forward-compatible
with future leaf kinds (e.g. a native signature lock) without changing the tree structure.
...
`script_root` field — not a `SpendCondition` enum variant and not a taproot-style key tweak. This
section specifies the tree and proof mechanism, which is independent of that choice.

#### 10. Output spend model: `spend_key` + `script_root` (decided integration)

The decided committed model replaces the inline `SpendCondition` field on the stealth output with
two separate committed fields — the authorization key and the MAST root (§9):
...
pub struct StealthUnspentOutput {
    ...
    /// MAST root over the alternative scripts (§9). `None` = no scripts.
    pub script_root: Option<Hash32>,
}
```

- **Invariant.** At least one of `spend_key` / `script_root` is `Some`; `{None, None}` is an
  unspendable (burn) output and is rejected at creation.
- `script_root == None` is a pure payment **or** — once the deferred tweak below lands — a
  tweak-hidden script; these share a single anonymity set.
- `script_root == Some(root)` is a **transparent, introspectable** script set / covenant, and is
  therefore a deliberately distinguishable class (see the rationale).
...
| Script-path | `script_root = Some(root)` | reveal one leaf + its `MerkleProof`; recompute and check the root (§9), then evaluate that leaf via §5 |
...
**Mapping the §1 enum.** `Signed(pk)` becomes the key path (`spend_key = Some(pk)`); `Script(s)`
becomes a script-path leaf with `id = 0` carrying `s`. The §2–§8 mechanism — ABI, T1/T2 signature
validation, the read-only sandbox, metering, and the covenant helpers — is unchanged and now applies
to the *revealed leaf*. Accordingly the `Outputs` introspection view (§4) and the covenant helpers
(§8) expose each output's `{spend_key, script_root}` in place of a single `SpendCondition`. The
mapping of the `AccessRule` variant — a dedicated leaf `id` versus an orthogonal output field —
**remains open**.
...
witness variant and accept a tweaked `spend_key` with a `None` `script_root`, opt-in per output, with
no format break. A NUMS internal key is needed only for the hidden-and-script-only case, and the
tweak hash MUST bind `P` (`H_tweak(P‖root)`) or forgery is trivial. Why the field ships first — and
why the tweak is not needed for covenants — is in the Rationale ("Why a committed `script_root`
field, not a key-path tweak").

### API changes

- `tari_template_lib_types`: the output's committed spend fields `spend_key:
  Option<RistrettoPublicKeyBytes>` and `script_root: Option<Hash32>` (§10) replacing the inline
  `SpendCondition`; the `SpendScript` leaf type; `SpendWitness` (key-path / script-path) and
...
- `tari_ootle_transaction`: extend `calc_stealth_statement_weight` so each stealth output's
  weight includes the serialized size of its `SpendCondition` (charging `Script` `args` at
  creation, `crates/transaction/src/v1/transaction.rs:284`).
...
### Why a committed `script_root` field, not a key-path tweak
...
stealth key, but extra plumbing on every check). A committed `script_root` field publishes the root
directly: the covenant reads it as plain data, with no key and no reconstruction, while each output
keeps an independent spend key.
...
stored substates is needed; only newly created outputs can carry a `Script` condition. The §10
output model — replacing the inline `SpendCondition` field with `spend_key` / `script_root` — is a
breaking re-encoding of the output; as Ootle is pre-launch this is acceptable and likewise requires
no migration of existing substates.
...
6. **T1 creation rejection** — creating an output whose `Script` names a missing function, a
   mutable (`is_mut == true`) function, a non-unit return, a missing `SpendContext` arg, or
...
9. **Value constraint** — "change must return to this condition" script enforced over
   `ctx.outputs()`.
10. **Weight** — an output with a large-`args` `Script` condition raises the *creating*
    transaction's weight (and fee) proportionally, versus an equivalent `Signed` output.
11. **Script tree (MAST)** — an output committing to a multi-leaf script root spends by revealing
    one leaf + inclusion proof: a valid proof for a committed leaf spends and evaluates that leaf; a
